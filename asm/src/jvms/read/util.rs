...
pub fn read_utf8_from_cp(index: usize, cp: &Vec<CPInfo>) -> AsmResult<String> {
    ...
    if tag != Constants::CONSTANT_Utf8 {
        return Err(AsmErr::IllegalArgument {
            info: format!("cannot read utf8 from constant pool, current cp tag: {}, index: {}", tag, index)
        });
    };
    if let Const::Utf8 { length, bytes } = info {
    ...
    Err(AsmErr::IllegalArgument {
        info: format!("cannot read utf8 from constant pool, index: {}", index)
    })
}