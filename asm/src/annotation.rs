...
impl AnnotationVisitor<'_> {
    ...
    fn visit(&self, name: &str, value: &AnnotationValue) -> Option<()> {
        self.delegated?.visit(name, value)
    }
    ...
    fn visit_annotation(&self, name: &str, descriptor: &str) -> Option<&Self> {
        self.delegated?.visit_annotation(name, descriptor)
    }
    ...
    fn visit_array(&self, name: &str) -> Option<&Self> {
        self.delegated?.visit_array(name)
    }
...
impl<'a> AnnotationVisitor<'a> {
    ...
    pub fn from(origin: &'a AnnotationVisitor) -> Self {
        Self {
